...
// renderHelpers 生成手写运行时辅助文件（仅在缺失时写入，升级时保留用户修改）。
// 语义严格对齐：
//   - ByteSpanExtensions.GetByteValue/SetByteValue/GetBoolean/SetBoolean
//   - ExtractString/WriteString（UTF-8、首 null 截断、定长零填充）
//   - C1-C4 包头（C2/C4 长度为大端双字节）
//
//go:generate 指令中的相对路径相对于本文件所在目录 internal/proto/<pkg>。
func renderHelpers(pkg string) string {
	return fmt.Sprintf(`// 手写运行时辅助，供 muprotogen 生成物调用；升级协议版本时不会被覆盖。
